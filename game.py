...
class Game:
        ...
        for i in range(MONSTER_COUNT):
            while True:
                ...
                if (self.maze.grid[y][x] == 0 and
                    abs(x - self.maze.start_pos[0]) + abs(y - self.maze.start_pos[1]) > 10):
    ...
    def handle_event(self, event):
        if event.type == pygame.KEYDOWN:
            ...
            if (self.game_state in [STATE_GAME_OVER, STATE_WIN] and 
                event.key == RESTART_KEY):
                self.new_game()
                self.add_transition_effect()
                return
        ...
        if self.game_state == STATE_PLAYING:
            ...
            self.player.handle_event(event, self.maze)
            new_pos = (self.player.x, self.player.y)
            
            if old_pos != new_pos and 'move' in self.sounds:
                self.sounds['move'].play()
                self.add_movement_particles(old_pos, new_pos)
    ...
    def update(self):
        ...
        for monster in self.monsters:
            ...
            if (int(monster.x) == int(self.player.x) and 
                int(monster.y) == int(self.player.y)):
    ...
    def update_effects(self, dt):
        ...
        if self.transition_alpha > 0:
            self.transition_alpha = max(0, self.transition_alpha - dt * 255 / TRANSITION_DURATION)
    ...
    def add_game_over_effect(self):
        ...
        self.particle_systems.append(
            ParticleSystem(center_x, center_y, PARTICLE_COUNT * 3, PARTICLE_LIFETIME * 2)
        )
    ...
    def draw_maze_and_entities(self):
        ...
        # Apply fog of war with smooth edges
        ...
        visibility_radius = PLAYER_VISION_RADIUS * CELL_SIZE
        
        # Create gradient visibility circle
        for radius in range(visibility_radius, visibility_radius - 20, -1):
            alpha = int((radius - (visibility_radius - 20)) * (255 / 20))
            pygame.draw.circle(fog_surface, (0, 0, 0, alpha),
                             (player_screen_x, player_screen_y), radius)
    ...
    def draw_ui(self):
        ...
        time_text = self.font.render(f"Time: {int(self.game_time)}", True, 
                                   UI_TEXT_COLOR)
        shadow_text = self.font.render(f"Time: {int(self.game_time)}", True, 
                                     (0, 0, 0))
        ...
        if self.player.speed_boost:
            ...
            self.screen.blit(boost_glow, (22, 62))
    ...
    def draw_title_screen(self):
        ...
        subtitle_glow = self.font.render(
            "Escape from AI",
            True,
            (min(255, 200 + glow), 0, min(255, 255 + glow))
        )
    ...
    def draw_game_over_screen(self):
        ...
        self.screen.blit(game_over_glow, 
                        (game_over_rect.x + 2, game_over_rect.y + 2))
    ...
    def draw_win_screen(self):
        ...
        time_text = self.font.render(
            f"Time: {int(self.game_time)} seconds",
            True,
            UI_TEXT_COLOR
        )
        restart_text = self.font.render("Press R to play again", True, 
                                      UI_TEXT_COLOR)
    ...
    def draw_pause_overlay(self):
        ...
        continue_text = self.font.render("Press P to continue", True, 
                                       UI_TEXT_COLOR)
